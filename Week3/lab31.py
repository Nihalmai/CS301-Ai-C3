...
class PriorityQueue:
    ...
    def pop(self):
        ...
        next_state = min(self.queue, key=lambda x: x.cost)
        self.queue.remove(next_state)
...
class Agent:
    def __init__(self, env, heuristic, initial_cost=0):
    ...
    def run(self):
        init_node = Node(parent=None, state=self.start_state, pcost=self.initial_cost, hcost=self.heuristic(self.start_state))
...
print("Average time for Agent 1:", t / 5)
...
print("Average time for Agent 2:", t / 5)
print("Number of nodes explored:", len(agent.explored))
print("Number of nodes in frontier:", len(agent.frontier))
